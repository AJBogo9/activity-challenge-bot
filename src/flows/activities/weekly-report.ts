...
// function buildNumericKeyboard(prefix: string, limit: number) {
//   const buttons = []
...
// async function handleNumericCallback(ctx: any, prefix: string, stateKey: string, multiplier: number, questionIdKey: string) {
...
// async function handleYesNoToggle(ctx: any, stateProp: string) {
//   if (await isNotCallback(ctx)) return
...
// async function handleYesNoAction(ctx: any, dataKey: string, multiplier: number) {
//   if (await isNotCallback(ctx)) return
...
// export const weekScoresWizard = new Scenes.WizardScene(
//   'week_scores_wizard',
//   async (ctx: any) => {
...
//     // Check if user can add points (based on last submission time)
//     const check = await canAddPoints(user.updated_at)
//     if (!check.canAdd) {
//       await ctx.reply((check as any).reason)
//       return ctx.scene.leave()
//     }
...
//       const rows = buildNumericKeyboard('sports', 8)
...
// weekScoresWizard.action('health_next', async (ctx: any) => {
//   if (await isNotCallback(ctx)) return
...
// weekScoresWizard.action(/toggle_(.+)/, async (ctx: any) => {
//   if (await isNotCallback(ctx)) return
...
// weekScoresWizard.action('health_submit', async (ctx: any) => {
//   if (await isNotCallback(ctx)) return
...
// weekScoresWizard.action('health_skip', async (ctx: any) => {
...
// weekScoresWizard.action('confirm_details', async (ctx: any) => {
//   try {
//     const { pointsData } = ctx.wizard.state
//     const titlePadding = 26
//     const valuePadding = 4
...
//     await pointService.addPoints(ctx.from.id.toString(), ctx.wizard.state.pointsData)
//     await ctx.editMessageText(message, { parse_mode: 'MarkdownV2' })
//     return ctx.scene.leave()
//   } catch (err) {
//     console.error('Error confirming details:', err)
//     await ctx.editMessageText(texts.actions.error.error)
//     return ctx.scene.leave()
//   }
// })

// weekScoresWizard.action('start_over', async (ctx: any) => {
//   await ctx.editMessageText('starting over!')
//   ctx.wizard.selectStep(0)
//   return ctx.wizard.steps[0](ctx)
// })

// weekScoresWizard.action('exit_wizard', async (ctx: any) => {
//   await ctx.editMessageReplyMarkup({})
